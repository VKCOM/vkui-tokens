...
import { colorStateMap } from '../../build/themeProcessors/expandColors/expandColors.js';
...
export function getStateFunctions(theme: ColorStateObject): StateFunctions {
	...
	return {
		stateHover(colorHover: string) {
			...
			return color(colorHover).mix(color(colorState), theme.toneValueHover).hex();
		},
		stateActive(colorActive: string) {
			if (colorActive === 'transparent') {
				return `rgba(${colorState},${theme.toneValueActive})`;
			}
			return color(colorActive).mix(color(colorState), theme.toneValueActive).hex();
		},
		stateFocus(colorFocus: string) {
			if (colorFocus === 'transparent') {
				return `rgba(${colorState},${theme.toneValueFocus})`;
			}
			return color(colorFocus).mix(color(colorState), theme.toneValueFocus).hex();
		},
		stateDisabled(colorDisabled: string) {
			return color(colorDisabled).alpha(theme.opacityDisable).rgb().string();
		},
	};
}
